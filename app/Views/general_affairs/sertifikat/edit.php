...
<div class="container-fluid">
    ...
    <div class="row">
        <div class="col-xl-12 col-sm-12">
            <div class="card">
                <div class="card-body">
                    <form class="needs-validation" novalidate action="<?= base_url('general_affairs/sertifikat_update/' . $list_data->id); ?>" method="post" enctype="multipart/form-data">
                        <?= csrf_field(); ?>
                        <div class="row">
                                    ...
                                    <div class="d-flex gap-2">
                                        <select name="tipe_sertifikat_id" class="form-select select2 flex-grow-1" data-toggle="select2" required>
                                            ...
                                            <?php foreach ($tipe_sertifikat as $row): ?>
                                                <option value="<?= $row->id; ?>" <?= ($row->id == $list_data->tipe_sertifikat_id) ? 'selected' : null ?>><?= $row->tipe_sertifikat; ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-xl-2 col-sm-2">
                                        <div class="mb-3 d-grid">
                                            <input type="hidden" name="old_file" class="form-label" value="<?= $list_data->file; ?>">
                                            <label for="" class="form-label">&nbsp;</label>
                                            <?php if ($list_data->file): ?>
                                                <a href="<?= base_url('sertifikat/' . $list_data->file); ?>" target="_blank" class="btn btn-warning"><span class="mdi mdi-file"></span></a>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                </div>
